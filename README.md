...
**记忆没生效。** 跨会话记忆靠 `hermes-agent` 提供的 SQLite FTS5 检索。这个依赖没装、或者版本对不上，程序会降级成无记忆运行，终端打一行提示，其他功能照常。

## 说明

主循环、工具面、前后端的业务逻辑都是自己写的。跨会话记忆那层把 `hermes-agent` 的 `SessionDB` 当库用，它自己的主循环没有复用。界面视觉是基于一份参考 UI 改写的，交互和业务逻辑是自研。代码规模（不含测试和调试脚本）后端约 1,750 行 Python，前端约 1,280 行。
